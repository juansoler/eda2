...
public class Depuradora {
	...
	public static void main(String[] args) throws IOException {
		...
		directorioEntrada = System.getProperty("user.dir") + File.separator +
				"src" + File.separator +
				"Practica" + File.separator + "pruebaN7.txt";
...
//			System.out.println("Zona gris: "+(cont++)+" "+aux);
	...
	private static HashSet<String> Greedy1v1(Sensor[][] matriz) throws IOException{
		...
		for(int j = 0; j < matriz[0].length; j++){
			for (int i = 0; i < medio; i++){
				...
				if(resultado.comprobarEstado(limitesSuperados) > 0){
					out.write("Problema en sensor: "+matriz[i][j].getNombre());
					out.write("\n");
					culpables.addAll(sensorEmpresas.get(matriz[i][j].getNombre()));
				}else{
					out.write("Sensor correcto: "+matriz[i][j].getNombre());
					out.write("\n");
				}
			}
		}
		...
		for(int i = matriz[medio].length-1; i >= 0; i--){
			...
			if(resultado.comprobarEstado(limitesSuperados) > 0){
				out.write("Problema en sensor: "+matriz[medio][i].getNombre());
				out.write("\n");
				culpables.addAll(sensorEmpresas.get(matriz[medio][i].getNombre()));
			}else{
				out.write("Sensor correcto: "+matriz[medio][i].getNombre());
				out.write("\n");
			}
		}
		...
		for(int i = 0; i < matriz[medio].length; i++){
			...
			if(comprobar != 0){
				...
				if(resultado.comprobarEstado(limitesSuperados) > 0){
					...
					out.write("\n");
				}else out.write("Las empresas que vierten a la calle "+(i+1)+" pueden trabajar");
				out.write("\n");
			}else{
	...
	private static void Greedy1v3() throws IOException{
		out.write("***************************************************");
		out.write("\n");
		...
		out.write("Metodo elegido Greedy1 v3");
		out.write("\n");
		out.write("\n");
		out.write("***************************************************");
		out.write("\n");
		...
		if(limitesSuperados.size()>0){
			out.write(Greedy1v3(matrizSensores).toString());
			out.write("\n");
		}
	...
	private static HashSet<String> Greedy1v3(Sensor[][] matriz) throws IOException{
		int medio = matriz.length/2;
		HashSet<String> culpables = new HashSet<String>();
		Sensor resultado = new Sensor();
		HashSet<Integer> callesCandidatas = new HashSet<Integer>();
