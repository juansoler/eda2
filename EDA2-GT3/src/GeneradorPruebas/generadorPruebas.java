...
public class generadorPruebas {
	private static HashSet<Integer> numeros = new HashSet<Integer>();
//	private static int[] numeros2 = null; // array auxiliar para dispersar las empresas contaminantes
	private static int contadorEmpresass = 0;  // va contando el numero de empresas generadas 
	...
	private static boolean casoPromedio = false;
	private static boolean peorCasoEsquinas = false;
	...
	public static void main(String[] args) {
		...
		int numeroAvenidas = 10;  // numero de avneidas
		...
		if (args.length == 0){
			...
			for (int x=0;x<numeroCalles*numeroAvenidas/2;x++)
				numeros.add((int) (Math.random()*(numeroCalles*numeroAvenidas))+1);
...
//			System.out.println(numeros.toString());
			...
			if (Integer.parseInt(args[7])==1) peorCasoEsquinas = true;
			numMaxEmpresasContaminantes = Integer.parseInt(args[8]);
			if (Integer.parseInt(args[9])==1) numMaxEmpresasContaminantes = 0;  // mejor
			if (Integer.parseInt(args[10])==1) numMaxEmpresasContaminantes = Integer.parseInt(args[1])*Integer.parseInt(args[2]); // peor
			if (Integer.parseInt(args[11])==1) {
				numMaxEmpresasContaminantes = Integer.parseInt(args[1])*Integer.parseInt(args[2])/2; // promedio
				casoPromedio = true;
			}
			
			for (int x=0;x<Integer.parseInt(args[1])*Integer.parseInt(args[2])/2;x++)
				numeros.add((int) (Math.random()*(Integer.parseInt(args[1])*Integer.parseInt(args[2])))+1);
			...
			for (int i=(65+numeroAvenidasN) ; i>65 ; i--){
				for(int j=1 ; j<numeroCalles+1; j++){
					...
					for (int k = 1; k <= numContaminantes ; k++){
						...
						if (peorCasoEsquinas && (i == 65+numeroAvenidasN) && !contaminadorAltoPorEmpresa) {
...
//								System.out.println("2");
					...
					contadorEmpresass++;
//					System.out.println("contadorEmpresas " + contadorEmpresass);
				}
				contador++;
				contaminadorAltoPorEmpresa = false;
//				peorCaso = true;
			...
			// ZONA SUR
						...
						for (int i=66 ; i<90 ; i++){
							...
							for(int j=1 ; j<numeroCalles+1; j++){
								...
								for (int k = 1; k <= numContaminantes ; k++){
									if (peorCasoEsquinas && (contador == numeroAvenidasN+1+numeroAvenidasS) && !contaminadorAltoPorEmpresa) {
...
//								System.out.println("contadorEmpresas " + contadorEmpresass);
	...
	public static double generarContaminante(){
	    ...
	    if (numeros.contains(contadorEmpresass) && casoPromedio && !contaminadorAltoPorEmpresa) {
//	    	System.out.println("contaminanteAlto 1");
	    	return generarContaminanteAlto(); 
	    }
	    if (!numeros.contains(contadorEmpresass) && casoPromedio || contaminadorAltoPorEmpresa) {
//	    	System.out.println("contaminanteBajo 2");
	    	return generarContaminanteBajo(); 
	    }
 
	    if (aleatorio) {
	    	if (peorCasoEsquinas || contaminadorAltoPorEmpresa) {
//		    	System.out.println("contaminanteBajo 3");
	    		return generarContaminanteBajo();
	    	}
			if (contadorEmpresasAltas >= numMaxEmpresasContaminantes || contaminadorAltoPorEmpresa) {
//		    	System.out.println("contaminanteBajo 4");

				return generarContaminanteBajo();
			}
			else {
//				System.out.println("contaminanteAlto 5");
				return generarContaminanteAlto();
			}
		}else {
//	    	System.out.println("contaminanteBajo 6");
			return generarContaminanteBajo();
		}
